...
class Grille:
    ...
    def initialiser_cases_par_defaut(self):
        ...
        # la variable case est un dictionnaire de case vide, il doit etre remplit à l'initialiser avec comme clé une position et comme valeur une case vide(sans jeton)
        case = {}
        #Boucle permettant de parcourir tout le dictionnaire vide en le remplissant
        for i in range(Grille.N_LIGNES):
            for j in range(Grille.N_COLONNES):
                case[j,i] = Case()
        return case
    ...
    def valider_coup(self, coup):
        ...
        if self.coup_dans_les_limites(coup) == False:
            #message est une variable de type string, on y initialise le message à signaler au joueur en cas de coup invalide
            message = "Coup en dehors des limites de la grille"
            return (False,message)
        elif self.colonne_est_pleine(coup) == True:
            message = "Coup dans une colonne déjà pleine"
            return (False,message)
        else:
            return (True,None)
    ...
    def coup_dans_les_limites(self, coup):
        ...
        '''
        return True if 0 <= coup < Grille.N_COLONNES else False
    ...
    def colonne_est_pleine(self, colonne):
        ...
        '''
        return True if self.cases[colonne, 0].jeton is not None else False
    ...
    def est_pleine(self):
        ...
        '''
        #la variable liste est la liste comportant les cases vides(sans jeton)
        liste = []
        for valeur in self.cases.values():
            if (valeur.jeton is not None):
                liste.append(valeur)
        if (len(liste) == 42):
            return True
        else:
    ...
    def jouer_coup(self, colonne, couleur):
        ...
        i = self.N_LIGNES
        while i > 0:
            est_dispo = self.case_est_disponible((colonne, i - 1))
            if (est_dispo == True):
                position = (colonne, i - 1)
                self.cases[position].mettre_jeton(Jeton(couleur))
                self.couleur_dernier_coup = couleur
                self.position_dernier_coup = position
                break
            i = i - 1
    ...
    def case_est_disponible(self, position_case):
        ...
        '''
        return True if (type(position_case) is tuple and position_case in self.cases and self.cases[
            position_case].jeton is None) else False
    ...
    def possede_un_gagnant(self):
        ...
        '''
        #la variable gagnant est une variable initialisée à False, elle passe à True si la grille possede un gagant.
        gagnant = False
        #directionPossible est une liste comportant les directions possibles
        directionPossible = [[1, 0], [1, 1], [1, -1], [0, 1]]
        #Pour chaque direction possible on parcout la boucle pour vérifier si la séquence gagnante admet au moins 4 éléments pour posseder un gagnant de la grille
        for i in directionPossible:
            if (self.position_dernier_coup is None):
                return False
            else:
                if len(self.obtenir_sequence_direction(self.position_dernier_coup, self.couleur_dernier_coup, i)) >= 4:
                    self.sequence_gagnante = self.obtenir_sequence_direction(self.position_dernier_coup, self.couleur_dernier_coup, i)
                    gagnant = True
        return True if gagnant else False
    ...
    def obtenir_sequence_direction(self, position, couleur, direction):
        ...
        Returns :
            ...
            position de départ
        '''
        #listeSequenceDirection est une liste de toutes les cases qui ont la meme couleur que la case de départ inclut elle meme suivant le vecteur direction et le vecteur direction inverse
        #newColonne est l'inverse de la colonne du vecteur direction
        #newLigne est l'inverse de la ligne du vecteur direction
        #newdirection est la nouvelle direction aprés avoir inversé le vecteur direction
        listeSequenceDirection=[]
        listeSequenceDirection.append(position)
        for valeur in self.obtenir_sequence_vecteur(position,couleur,direction):
            listeSequenceDirection.append(valeur)
        newColonne = direction[0]*(-1)
        newLigne = direction[1]*(-1)
        newdirection = [newColonne,newLigne]
        for valeur in self.obtenir_sequence_vecteur(position,couleur,newdirection):
            listeSequenceDirection.append(valeur)
        return listeSequenceDirection
    ...
    def obtenir_sequence_vecteur(self, position, couleur, vecteur_direction):
        ...
        '''
        #listetuple est une liste de tous les positions qui ont la meme couleur que la position de départ suvivant le vecteur direction
        # newColonne est un entier initialiser par la position de la colonne de la case de départ + celle du vecteur direction
        # newLigne est un entier initialiser par la position de la ligne de la case de départ + celle du vecteur direction
        listetuple = []
        while position in self.cases.keys() and self.cases[position].jeton.couleur == couleur:
            newColonne = position[0] + vecteur_direction[0]
            newLigne = position[1] + vecteur_direction[1]
            listetuple.append(position)
            position = (newColonne, newLigne)
            if position in self.cases.keys() and self.cases[position].jeton is None:
                del listetuple[0]
                return listetuple
        del listetuple[0]
        return listetuple
    ...
    def obtenir_coups_possibles(self):
        ...
        '''
        #non_pleine est la liste de toutes les colonnes non-pleine de la grille
        non_pleine = []
        for i in range(0,7):
            if(self.cases[i,0].jeton is None):
                non_pleine.append(i)
        return non_pleine
    ...
    def get_case(self, position):
        ...
        '''
        return self.cases[position] if position in self.cases.keys() else None
    ...
    def convertir_en_chaine(self):
        ...
        for i in range(0, 7):
            for j in range(0, 6):
                if self.cases[i,j].jeton is not None:
                    chaine += str(i) + "," + str(j) + ","+self.cases[i,j].jeton.couleur+"\n"
                else:
                    chaine += ""
        return chaine
    ...
    def charger_dune_chaine(self, chaine):
        ...
        for i in range(len(chaine1)-1):
            ...
            if (case[4] == "j"):
                ...
                color += "rouge"
            self.cases[colonne,ligne].mettre_jeton(Jeton(color))
